...
// A lot of flags
    ...
    window->state  = (int) (state & 0xFFFFFFFF);
    window->focus  = FALSE;
...
// Overlapped
    if (type == WT_OVERLAPPED)
    {
        ...
        __w = 
            (void *) doCreateWindow ( 
                         WT_SIMPLE, style, status, state, (char *) name_local_copy,
                         x, y, width, height, 
                         (struct gws_window_d *) pWindow, 
                         desktopid, 
...
// Podemos usar o esquema padrão de cores ...
    if ( type == WT_EDITBOX || type == WT_EDITBOX_MULTIPLE_LINES )
    {
        ...
        __w = 
            (void *) doCreateWindow ( 
                         WT_SIMPLE, 0, status, state, (char *) name_local_copy, 
                         x, y, width, height, 
                         (struct gws_window_d *) pWindow, 
                         desktopid, 
                         frame_color, client_color, 
...
// Podemos usar o esquema padrão de cores ...
    if (type == WT_BUTTON)
    {
        ...
        __w = 
            (void *) doCreateWindow ( 
                         WT_BUTTON, 0, status, state, (char *) name_local_copy, 
                         x, y, width, height, 
                         (struct gws_window_d *) pWindow, 
                         desktopid, 
                         frame_color, client_color, 
