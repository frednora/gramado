    ...
    unsigned long long2 )
{
    ...
    */
    ...
    case MSG_PAINT:
        // Redraw child windows if needed
...
// which describe the client area rectangle of the main window.
    ...
    // Restart button inside client area
    restart_button = (int) gws_create_window(
        client_fd,
        WT_BUTTON,
        BS_DEFAULT,
        1,
        "Restart",
        20,          // left offset inside client area
        (wi.cr_height -40)/2,           // top offset inside client area
        wi.cr_width / 4,          // width relative to client area
        40,                       // fixed height
...
//
    ...
    // Shutdown button inside client area
    shutdown_button = (int) gws_create_window(
        client_fd,
        WT_BUTTON,
        BS_DEFAULT,
        1,
        "Shutdown",
        (wi.cr_width / 2), // place it to the right
        (wi.cr_height -40)/2,
        wi.cr_width / 4,
        40,
        main_window, 0, COLOR_GRAY, COLOR_GRAY );
