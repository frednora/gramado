...
Each branch in the switch(c) inside css_yylex() represents a transition 
        ...
        //case '.':
            ...
            if (c == '0'){
                ...
                *p = c;  // Save it into the buffer
                ...
                // #bugbug
                printf ("css_yylex: FAIL expected x in constant in line %d", 
                    LexerInfo.current_line );
                exit (1);
 ...
 *     This is a worker, called by csslex_initialize().
 */
static int __lexerInit(void)
{
    register int i=0;
