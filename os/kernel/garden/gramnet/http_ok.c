...
#define FILEID_ROOT  1
#define FILEID_ABOUT  2
#define FILEID_FAVICON_ICO  3
#define FILEID_INDEX_HTML  4
// ...
...
static int __build_response_view(int method_id, int file_id, int parameter_id);
...
// 6   → /index.html
    ...
    {
        HTTP_REQUEST_INFO.FileId = FILEID_ROOT;
        HTTP_REQUEST_INFO.ParameterId = 0;
    }
    ...
    {
        HTTP_REQUEST_INFO.FileId = FILEID_ABOUT;
        HTTP_REQUEST_INFO.ParameterId = 0;
    }

    if ( gramado_strncmp(http_uri, "/favicon.ico", 12) == 0 )
    {
        HTTP_REQUEST_INFO.FileId = FILEID_FAVICON_ICO;
        HTTP_REQUEST_INFO.ParameterId = 0;
    }

    if ( gramado_strncmp(http_uri, "/index.html",  11) == 0 )
    {
        HTTP_REQUEST_INFO.FileId = FILEID_INDEX_HTML;
        HTTP_REQUEST_INFO.ParameterId = 0;
    }
...
// matches the real body size instead of a hand-typed literal.
    ...
    switch (file_id) 
    {
        ...
        case FILEID_INDEX_HTML:
            strcat(resp, "Content-Type: text/html; charset=utf-8\r\n");
            break;

        case FILEID_FAVICON_ICO:
