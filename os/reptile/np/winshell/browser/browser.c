// browser.c
// This is gonna be the UI of a web browser for Gramado OS.
// Environment: ring 3
...
struct browser_initialization_d  BrowserInitialization;
...
// Program name
static const char *program_name = "BROWSER";
// Addressbar title
...
static void browserDrawStatusBar(void)
{
    ...
    char status[64];
    const char *default_url = "/default.html";
    const char *default_page_state = "ok";

    // #test: Plug the default strings
...
// Pump
    ...
    if (e->type < 0)
        return;
// Dispatch
    browserProcedure( fd, e->window, e->type, e->long1, e->long2 );
}
...
//
    ...
    BrowserInitialization.file_loaded_at_initialization = TRUE;
    return 0; // success
...
// Draw buffer's content
    if (BrowserInitialization.file_loaded_at_initialization == TRUE)
    {
        editorRedrawBuffer(dc00);
    }
...
int browser_initialize(int argc, char *argv[])
{
    int status = -1;

    BrowserInitialization.initialized = FALSE;
    BrowserInitialization.file_loaded_at_initialization = FALSE;
