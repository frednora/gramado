...
static int head(Hash *t, Object *ref)
{
    ...
    // STUFF
        ...
        lua_reportbug("head: unexpected type to index table");
        return -1;
    };
}
...
// #ps: Gramado OS is still not good with free().
...
Object *lua_hashdefine(Hash *t, Object *ref)
{
    ...
    if (n == NULL)
    {
        n = new(Node);
        if (n == NULL){
            lua_error("not enough memory");  return NULL;
        }
        n->ref = *ref;
        tag(&n->val) = T_NIL;
    ...
    // Return pointer for an object structure
    return (&n->val);
}
...
** Given an array object and a reference value, 
** return the next element in the hash.
** This function pushs the element value and 
** its reference to the stack.
*/
