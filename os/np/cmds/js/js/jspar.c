...
// parse_log:
// Grammar rule: log → "log" "(" string ")" ";"
// - Enforces parentheses around the string and a trailing semicolon.
// - Semantic action: creates an object with OP_LOG opcode.
...
static int parse_log(int token)
{
    ...
    if (c != TK_SEPARATOR || gramado_strncmp(real_token_buffer, "(", 1) != 0) {
        printf("parse_log: expected '('\n");
        exit(1);
    }

    c = js_yylex();
    if (c != TK_STRING) {
        printf("parse_log: expected string\n");
        exit(1);
    }
...
//
    ...
    if (!o) { 
        printf("parse_log: malloc failed\n"); 
        exit(1); 
    }
    ...
    // consume ')' and ';'
    c = js_yylex();
    if (gramado_strncmp(real_token_buffer, ")", 1) != 0) { printf("parse_log: expected ')'\n"); exit(1); }
    c = js_yylex();
    if (gramado_strncmp(real_token_buffer, ";", 1) != 0) { printf("parse_log: expected ';'\n"); exit(1); }
...
// It repeatedly calls the lexer (js_yylex) to obtain tokens,
...
// Called by compiler().
int jspar_initialize(void)
{
    //printf ("jspar_initialize:\n");
