...
Each branch in the switch(c) inside html_yylex() represents a transition 
...
struct htmllex_info_d  LexerInfo;
...
static int __jslex_skip_white_space(void);
...
int check_newline ()
{
    ...
    while (1)
    {
	   //Entramos nessa função porque encontramos um '\n'.		
		...
		//se for quanquer outra coisa também deixaremos 
        // o __jslex_skip_white_space tratar
		return (int) c;
    ...
    for (;;)
    {
        switch (c)
        {
            ...
            case '/':
				...
				//printf("__jslex_skip_white_space: todo: depois da barra / .");
				//exit(1);
...
again:
    ...
    c = (int) __jslex_skip_white_space();
    ...
    switch (c)
    {
        ...
        case '_':
            ...
            // After building the identifier in real_token_buffer
            ...
            if ( TokenSize == 5 && 
                 gramado_strncmp( real_token_buffer, "print", 5 ) == 0 )
            {
        ...
        //case '.':
            ...
            if (c == '0'){
                ...
                *p = c;  // Save it into the buffer
                ...
                // #bugbug
                printf ("html_yylex: FAIL expected x in constant in line %d", 
                    LexerInfo.current_line );
                exit (1);
 ...
 *     This is a worker, called by htmllex_initialize().
 */
static int __lexerInit(void)
{
    register int i=0;
